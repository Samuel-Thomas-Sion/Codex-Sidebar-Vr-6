...
import { cn } from './utils'; // we can just define a simple cn
...
export function ClosingPlasma({
  ...
  const isDarkRef = ml.useRef(1); // Force dark for this app
  ...
  return (
    ...
    >
      ...
      />
      {children && <div className="relative z-10 h-full w-full" style={{ position: "relative", zIndex: 10, height: "100%", width: "100%" }}>{children}</div>}
    </div>
  );
}
